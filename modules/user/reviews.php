...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($book_id && $rating && !empty($comment)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                $message = "<div class='alert alert-success'>Review submitted successfully and is pending approval.</div>";
            } else {
                $message = "<div class='alert alert-danger'>Failed to submit review. Please try again.</div>";
            }
            $stmt->close();
        }
        $check->close();
    } else {
        $message = "<div class='alert alert-warning'>Please fill all fields before submitting.</div>";
    }
}
